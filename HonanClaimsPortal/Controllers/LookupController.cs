...
namespace HonanClaimsPortal.Controllers
{
    ...
    public class LookupController : Controller
    {
        ...
        public ActionResult PolicyNewClaimAjaxHandler(jQueryDataTableParamModel param,string team,string ocNum, string accountId,string claimType,string incidentDate,string policyClass,bool activePolicies)
        {
            lookupServices = new LookupServices();
            List<PolicySimple> objectList = new List<PolicySimple>();

            if((team == "Risksmart Property" || team == "Property Claims") && string.IsNullOrEmpty(ocNum))
                objectList = new List<PolicySimple>();
            else
                objectList =
                lookupServices.GetPoliciesNewClaim(param.sSearch, accountId, claimType,incidentDate,policyClass,activePolicies);
            ...
            foreach (PolicySimple item in filteredRecords)
            {
                string[] arry = new string[] { item.PolicyId, item.PolicyNo, item.PolicyClass, item.PolicyStatus, item.PolicyInception, item.PolicyExpiry, item.Address1, item.Address2,
                    item.Suburb, item.State, item.Postcode, item.PolicyExpiry, item.AccountManager,
                   item.Excess.ToString(),
                    item.UnderwriterId,item.UnderwriterName,item.Insured_Name,item.PolicyId,item.AccountManagerCode};
        ...
        public ActionResult OcAjaxHandlerNewClaim(jQueryDataTableParamModel param)
        {
            lookupServices = new LookupServices();
            List<CRMOCNumSimple> objectList = new List<CRMOCNumSimple>();
            ...
            //{
            objectList =
                lookupServices.TeamGetOcNumLookupByPolicyNewClaim(param.sSearch, "");
            // }
            ...
            foreach (CRMOCNumSimple item in filteredRecords)
            {
                string[] arry = new string[] { item.OCNum,item.Account,item.AccountId};
                aData.Add(arry);
            }
